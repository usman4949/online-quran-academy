...
<script>
    ...
    $(document).on('click', '.delete_icon', function() {
        ...
        var $likeTourElement = $(this);
        var product_id = $likeTourElement.attr('data-id');

        //  var product_id = $(this).attr('data-id');
        $.ajax({
            url: "<?php echo base_url('like_tours'); ?>",
            type: 'get',
            data: {
            ...
            success: function(html) {
                if (html.status == 1) {
                    // $likeTourElement.css('color', 'red');
                    $likeTourElement.parents('.mywishlist_card').html('');
                    //$likeTourElement.html(`<i class="fa fa-heart" style="color:red" id="like_tour" data-id="${product_id}"></i>`);
                }
            }
        });
